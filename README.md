...
| Record arrivals/departures and correct attendance | Yes | Enabled by default; owner can turn off | Never |
...
| View own monthly calendar | Via worker history | Via worker history | Enabled by default; owner can turn off |
...
Workers never mark or edit attendance, even if an old app calls the former self-check-in/out endpoints. Those endpoints now return 403.
...
7. Owner: **Shop settings** → enable **Managers can add workers**, optionally **Managers can edit and deactivate workers**, choose **Check-in and check-out**, and save.
8. Manager: **Manage workers → Add worker** now appears. Add another worker, then record **Mark in** and **Mark out** in Today’s attendance.
...
12. Try Manager/Worker login using an unadded number: it is rejected. **Profile → Sign out** revokes the current session.

Owners can create several shops and users with multiple active memberships get a shop switcher. The same mobile may have different roles in different shops; the login portal and active membership both constrain access. To give a manager a personal attendance record as well, add a separate worker membership and use the Worker portal for their own calendar.
...
- **WorkerProfile** (`worker_profiles`): membership-specific name. Unique `membership_id`. Its ID equals the worker membership ID, which is also the API's stable `worker_id`.
...
| GET | `/shops/{shop_id}/attendance/today` | Owner/authorised manager attendance overview |
...
| GET | `/shops/{shop_id}/me/attendance/today` | Worker today and active shift |
| GET | `/shops/{shop_id}/me/attendance?month=YYYY-MM` | Worker's own calendar days, summary, joining date and today |
...
- `mobile/App.tsx`: authenticated role-specific native stack navigation.
...
- 15 backend integration tests passed against MongoDB 7.0.2.
- 3 Playwright browser tests cover the full owner/manager/worker flow and staff login eligibility.
